package com.example.nestedscroll.e_perfect_nestedscroll;
...
import com.example.common.fragment.NestedLogRecyclerView;
import com.example.common.utils.FlingHelper;
...
public class NestedScrollLayout extends NestedScrollView {
    ...
    private void init() {
        ...
        setOnScrollChangeListener(new View.OnScrollChangeListener() {
            ...
            public void onScrollChange(View v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                ...
                Log.i(TAG+"onScrollChange",  scrollY + "::getChildAt::" + getChildAt(0).getMeasuredHeight() + "::v.getMeasuredHeight() -> " + v.getMeasuredHeight());
    ...
    public void onNestedPreScroll(@NonNull View target, int dx, int dy, @NonNull int[] consumed, int type) {
        Log.i(TAG, getScrollY() + "::onNestedPreScroll::" + topView.getMeasuredHeight() + "::dy -> " + dy);
        // 向上滑动。若当前topview可见，需要将topview滑动至不可见
        boolean hideTop = dy > 0 && getScrollY() < topView.getMeasuredHeight();
        if (hideTop) {
            scrollBy(0, dy);
            //如果此处注释掉，recycleView和nestScrollView都要滑动
            consumed[1] = dy;
        }
    }
